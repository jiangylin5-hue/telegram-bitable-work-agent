...
| Full backend suite pass | completed locally through redacted runtime summary pass | `pytest tests -q`: 255 passed / 17 skipped |
...
| Stage05 deployed to Tencent Cloud staging | pending | commit and server evidence |
| Stage05 migration applied | pending | `alembic current` |
| Real OpenRouter enabled server-side | pending | redacted env evidence |
| Real OpenRouter mode reaches runtime containers | pending | redacted `api` and `worker` settings summaries showing `LLM_ENABLED=true`, `AGENT_WORKFLOW_MODE=real_openrouter`, OpenRouter key present, raw prompt/response persistence disabled and provider disabled |
| Provider remains disabled | pending | redacted env evidence |
| Restricted test send allowlist configured | pending | redacted env evidence |
| Mixed Chinese/English Telegram message received | pending | update id/message id |
| AgentRun records real OpenRouter metadata | pending | `agent_runs` evidence |
