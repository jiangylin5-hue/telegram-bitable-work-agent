...
| Router schema validation | completed locally | `pytest tests\unit\test_stage05_router_schema.py -v`: 6 passed |
...
| Full backend suite pass | completed locally through runtime wiring fix | Latest `pytest tests -q`: 258 passed / 17 skipped |
...
| Stage05 deployed to Tencent Cloud staging | partial; redeploy pending | Reviewed commit `17043e8176b25e85fcc022a259bd5a99ee473690` was deployed for first rehearsal. Runtime wiring fix is newer and must be committed/deployed before this row can pass. |
| Stage05 migration applied | completed for first rehearsal | Staging `alembic current` reached `20260707_0016`; no new migration is expected for the runtime wiring fix |
...
| Mixed Chinese/English Telegram message received | completed for first rehearsal | Message id `f17a2214-7f6c-4474-9361-6a586458f93b`, Telegram message `6`, redacted chat `769***919`, trace `tg:184365904`, binding `bound`, processing/outbox `processed` |
| AgentRun records real OpenRouter metadata | blocked by first rehearsal defect; pending retry | No AgentRun was produced because the worker runtime did not inject Stage05 workflow. Local fix exists and must be redeployed before retry. |
