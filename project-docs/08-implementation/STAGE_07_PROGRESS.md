...
- Local branch commits 37b1e95 (safe transport contract) and c3688da (atomic relation initializer) remain narrow implementation evidence only. Uncommitted lookup work is paused and must not be used as verification or acceptance evidence. The next gate is user review of the reconciled document package before code resumes.
