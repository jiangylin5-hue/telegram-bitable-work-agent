...
P3 and F1 are closed as bounded `implemented-local` substages: their real PostgreSQL and browser evidence is recorded above, and delayed workspace/view replacement remains covered by the application scope-isolation test because the modal correctly prevents an unsafe forced switch. The next implementation work is not authorised yet: F2 relation/lookup, V1 additional views and import/template each require a separately discussed and approved safe browser/API/permission contract before any code starts. Governance and Package 4 remain separate contract gates.
