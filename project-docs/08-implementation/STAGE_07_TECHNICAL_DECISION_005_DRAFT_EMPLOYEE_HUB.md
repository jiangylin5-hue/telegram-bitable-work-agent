...
| `record_change_drafts` | partial/composite index candidate `(base_id, created_at DESC) WHERE status = 'pending_confirmation'` | supports the only S5 queue list; its creation requires the measured index evidence in the companion complex-index document. |
...
| `GET /mini-app/bases/{base_id}/drafts` | cursor | safe pending/terminal list summaries only |
