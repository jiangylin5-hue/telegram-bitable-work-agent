...
| 05.6 Task12 Staging Runtime Wiring Fix | completed locally; redeploy pending | Approved staging rehearsal proved deployment, migration, health, redacted real-mode env and Telegram inbound/binding, but exposed missing Stage05 workflow injection in `stage03_runtime.py`; local fix and regressions pass, staging redeploy is required |
| 05.6 Task12 Router Prompt Contract Fix | completed locally; redeploy pending | Staging after runtime wiring fix proved real OpenRouter AgentRun evidence but failed schema validation; local prompt contract/example fix passes targeted, focused and full backend tests before any new staging attempt |
| 05.6 Local Real OpenRouter Workflow Gate | completed locally; staging retry pending | `backend/scripts/stage05_local_real_workflow.py` passed real OpenRouter routed-draft and risk/manual-review scenarios after entity-key prompt guidance was added |
