...
- Current Progress Update: 2026-07-08 Local real OpenRouter smoke was added as a mandatory pre-staging gate and passed after the router prompt-contract fix. The smoke used temporary local env only and produced redacted evidence: `ok=true`, `model_provider=openrouter`, `model_name=openrouter/auto`, `prompt_version=stage05-router-v1`, request id present, usage present, `intent_types=["recharge","bm_invite"]`, `overall_confidence=0.7350`, `requires_manual_review=true`, with manual-review reasons for missing BM invite details/local smoke context. This is an acceptable conservative output because the model did not invent missing invite details and the response parsed through `RouterResult`. The local key must be cleared after the smoke; the fix still requires commit, push, staging redeploy and fresh Telegram evidence.
...
| 05.6 Local Real OpenRouter Smoke Gate | completed locally; staging retry pending | `STAGE_05_LOCAL_OPENROUTER_ENV_SMOKE.md` defines temporary-key local real LLM smoke; redacted pass evidence shows schema-valid conservative `recharge` + `bm_invite` output with manual review |
...
Status: completed locally with deterministic tests and local real OpenRouter smoke; commit, push and redeploy pending
Completed: After runtime wiring fix c9347dfb780e2af36c89894d5e2f8cd574f479f9 was deployed, staging message 0c466049-309a-40f8-805a-6e682937de1e proved the worker now invokes Stage05 and records a real OpenRouter AgentRun. The run used model_provider=openrouter, model_name=openrouter/auto and prompt_version=stage05-router-v1, but failed with agent_output_invalid because the real model output did not match RouterResult. Strengthened message_intake_router.py so the system prompt requires the exact top-level RouterResult shape, disallows flat top-level intent fields, lists allowed intent types, requires arrays/objects for empty collections, and includes a schema-valid example response. Added tests that assert the prompt contract is present and the embedded example validates against RouterResult. Added STAGE_05_LOCAL_OPENROUTER_ENV_SMOKE.md and linked it from Stage05 docs so local real LLM smoke becomes a required gate before staging retries.
Changed files: backend/app/agents/message_intake_router.py; backend/tests/unit/test_stage05_router_schema.py; project-docs/08-implementation/STAGE_05_LOCAL_OPENROUTER_ENV_SMOKE.md; project-docs/08-implementation/README.md; project-docs/08-implementation/STAGE_05_TEST_PLAN.md; project-docs/08-implementation/STAGE_05_OPERATIONS_RUNBOOK.md; project-docs/08-implementation/STAGE_05_PROGRESS.md; project-docs/08-implementation/STAGE_05_ACCEPTANCE_CHECKLIST.md; project-docs/08-implementation/STAGE_05_FINAL_ACCEPTANCE_REPORT.md.
Tests run: pytest tests/unit/test_stage05_router_schema.py tests/integration/test_stage05_agent_workflow.py -q; pytest tests -k stage05 -q; git diff --check; pytest tests -q; local real OpenRouter smoke from STAGE_05_LOCAL_OPENROUTER_ENV_SMOKE.md.
Test result: Targeted router/workflow tests passed 15/15. Focused Stage05 tests passed 86/86 selected with 190 deselected. Full backend suite passed 259/259 with 17 online PostgreSQL smoke tests skipped because STAGE02_ONLINE_DATABASE_URL is not configured. git diff --check reported no whitespace errors and only Windows LF-to-CRLF warnings. Local real OpenRouter smoke returned ok=true with model_provider=openrouter, model_name=openrouter/auto, prompt_version=stage05-router-v1, request_id_present=true, usage_present=true, intent_types=["recharge","bm_invite"], overall_confidence=0.7350 and requires_manual_review=true.
Not done: This prompt-contract fix is not yet committed, pushed, deployed to Tencent Cloud or proven with a new staging Telegram message. Customer reply was not selected in the local smoke; the result is still acceptable because the model conservatively requested manual review for ambiguous/missing details instead of inventing them.
Risks / follow-up: A real model can still produce invalid output, but the contract is now explicit, locally schema-checked and locally smoke-tested against OpenRouter. After commit/push/redeploy, send a new private allowlisted test message and verify intent_status reaches routed or manual_review with succeeded AgentRun/draft evidence. If it still fails, capture only redacted AgentRun/audit evidence and adjust locally again before staging.
Next subphase: Commit/push prompt-contract fix, redeploy staging, and retry Task12 with a fresh Telegram test message.
```
