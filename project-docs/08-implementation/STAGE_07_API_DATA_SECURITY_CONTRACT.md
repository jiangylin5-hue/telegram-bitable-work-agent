...
| `GET /bases/{base_id}/views` | legacy saved-view summary `{ id, base_id, table_id, name, view_type, status }`; a V1 row additionally has only safe `scope`, `caller_access_level`, `is_default` | resolves Base ownership then requires `table.read`; for V1 views it additionally applies the resolved owner/restricted-grant/system-default view ACL before emitting a summary, while legacy non-V1 list behavior remains unchanged; excludes owner identity, view config and permission policy |
...
| `GET /views/{view_id}/presentation` | normalized saved-view semantics `{ view_type, visible_field_keys, group_by_field_key?, date_field_key?, form_field_keys }` | requires `record.read` plus view-resource visibility; V1 rows first resolve the same owner/restricted-grant/system-default read ACL and use the safe V1 projection, while legacy rows retain their existing resource-policy check; every returned field key must pass field read policy; raw config and policy are excluded |
...
The existing `GET /bases/{base_id}/views` list is the Canvas tab source, not a legacy bypass for V1 ACL. After the normal Base/table authorization it must retain a V1 summary only when the same effective V1 read decision permits that caller. A private owner view and a restricted view without an active grant are omitted, not represented by a placeholder or a denial row; system default and active-granted views remain ordinary safe summaries. A V1 row carries the already documented safe-summary `scope`, caller access level and default marker so the browser can choose the V1 Builder path after a refresh without probing legacy raw views. This adds no endpoint, request parameter, role or policy field.
