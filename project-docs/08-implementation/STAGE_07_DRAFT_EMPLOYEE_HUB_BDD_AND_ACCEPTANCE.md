...
| --- | --- | --- |
| safe summary projection and context intersection | backend unit plus local PostgreSQL run: `16 passed`; malicious citation fields and unknown record IDs are dropped, cross-Base view is denied before runtime, terminal concurrency is locked, and a post-creation field revoke hides the diff/value then disables confirmation | real provider and stale context matrix remain absent |
