...
- Implementation progress: bootstrap, Workspace Home, the initial Base-open dependency tree, saved-view switching and record-detail opening are migrated. The Base-open tree covers Base tables/views, the default table schema, the default view presentation and its first cursor window. Cursor follow-up pages and mutations remain explicit later slices; this decision is not yet fully implemented.
...
- This evidence does **not** claim migration of cursor continuation or mutation invalidation. Those paths still use their existing local state until independently covered by a red/green slice.
