...
- S6.1 Current Implementation: TD007 Option A is `partial-local`. Official `initData` verification, binding-backed identity, opaque short-lived pointer migration/resolver and Mini App in-memory handoff are implemented with focused backend/Mini App and disposable PostgreSQL evidence. Four-width target-state Browser proof, real Telegram smoke, external delivery, staging/production and total Stage07 acceptance remain unaccepted.
