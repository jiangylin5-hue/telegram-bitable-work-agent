...
- Current Progress: 2026-07-12 Package 1 and bounded Package 2 paths through F2 are `implemented-local`; their local test, PostgreSQL and Browser evidence remains recorded below. The user approved V1's comprehensive design and implementation plan. V1-1 through V1-12 are `partial-local`: migration `20260711_0022`, durable owner/scope/version and `ViewMemberGrant` primitives, strict command/safe-read schemas, canonical validation, idempotent private initialization, effective owner/editor/viewer access, atomic grants, deterministic server filter/group/sort pagination, five safe routes, safe select `filter_values`/F2 `field_id` projection, Base-list/direct-presentation ACL resolution, safe list markers, local PostgreSQL evidence, scoped transport, Canvas rereads, fixed-detail suppression and 390px guards are implemented. Task 12 adds actual disposable-fixture Browser observations at 1440/1280/430/390 for Grid/Kanban/Calendar/Form, owner grant and `409` reread with console `[]`, but does not claim a real-backend Browser, every role/negative state, Telegram, staging or production. Measured `EXPLAIN` retains both optional non-unique access/list indexes as deferred; no index migration exists. Imports/templates, Package 3 and Package 4 remain incomplete or contract-gated.
