...
- Substage Delivery Update: Stage07 now uses the S0--S6 coherent-substage roadmap. S5 has an approved TD005 decision/design/BDD/SDD/module/index/plan package and a proposed TD006 context-binding decision package; S6 Telegram/final acceptance remains independently contract- or evidence-gated. See [Substage Delivery Roadmap](STAGE_07_SUBSTAGE_DELIVERY_ROADMAP.md).
...
The following remain a separate, unapproved Package 4 contract gate. TD005 Option A is the sole approved exception: it is a bounded contact/context/draft-review adapter over existing Stage06 resources; its approved implementation may not expand the listed Package 4 boundary.
