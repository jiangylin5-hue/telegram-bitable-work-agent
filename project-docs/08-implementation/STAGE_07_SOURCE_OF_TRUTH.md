...
- Next proposed substage: TD010 Option A documents a Base-bound digital employee management plane. It proposes a versioned `draft|active|paused` lifecycle, explicit optional member-use grants and safe management adapters while preserving existing runtime/Base/view authority. It requires a schema migration, API contract and permission-behavior approval; this document package authorizes no code.
