...
- Current Progress: approved F2 backend transport/initializers/read-write/lookup work and the bounded Mini App Picker, create/direct-edit, safe-rendering and Builder paths have local commits. The frontend has fresh `18` files / `92` tests and a passing production build after independent reviews. A disposable local Browser fixture exercised the primary Builder/Picker/create/direct-edit/read-only-lookup flow at 1440/1280/430/390, then a desktop negative fixture covered nested lookup output, replay, 409 builder lock, allowlisted invalid feedback and candidate 403 with final console logs `[]`. A targeted mobile negative run observed 430px conflict lock, replay `200` followed by authoritative reread before the new relation header rendered, 390px candidate 403 containment, and 390px fixed `lookup_target_incompatible` feedback without the fixture's server detail; the local-origin console `error`/`warn` scan was `[]`. The full backend regression is `463 passed, 27 skipped`; F2 PostgreSQL remains skipped because its authorised URL is absent. It is not the complete F2 Browser or PostgreSQL matrix, and no final F2 acceptance is claimed.
...
| F2-A10 | four-width actual Mini App QA | local fixture + browser screenshots + console scan | partial-local: primary flow passed at 1440/1280/430/390; desktop negative states passed with console `[]`; mobile `430x844` conflict lock and replay `200` followed by authoritative reread passed, while `390x844` candidate-403/global-denied and `422 lookup_target_incompatible` fixed-feedback paths passed. The latter rendered only `目标字段与聚合方式不兼容。` and suppressed the fixture's server detail; local-origin console was `[]`. Evidence remains fixture-local, and F2 PostgreSQL plus final requirement audit remain open. |
