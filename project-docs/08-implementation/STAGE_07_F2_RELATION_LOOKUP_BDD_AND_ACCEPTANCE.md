...
- Current Progress: the approved bounded F2 transport, relation/lookup initializers, safe relation/lookup reads, relation write rechecks, protected Picker/create/direct-edit, safe Canvas/Detail rendering and separate Builder are `implemented-local`. Fresh evidence is Mini App `18` files / `93` tests and production build, dedicated authorised-disposable PostgreSQL `6 passed` (relation rollback/concurrency/replay, lookup rollback, dependency guards, and all seven fixed aggregation projections), and full backend `477 passed, 17 skipped`. The 17 skips are only historical Stage02 online-database smoke tests without `STAGE02_ONLINE_DATABASE_URL`. Local Browser fixtures covered the primary Builder/Picker/create/direct-edit/read-only-lookup flow at 1440/1280/430/390, desktop nested/replay/conflict/invalid/denied states, and mobile negative states. A final local Browser pass at 1440 and 390 verified all seven safely rendered aggregate values, direct relation selection/removal/reselection, `Current record` excluded from same-table candidates, `Other record` retained, and a local-origin console scan of `[]`. This is bounded local F2 acceptance evidence only: it is not Telegram, staging, production or whole-Stage07 acceptance.
...
| --- | --- | --- | --- |
| F2-A01 | relation same-Base atomic initializer | unit/API replay/conflict/rollback + PostgreSQL | implemented-local: API/unit evidence plus authorised disposable PostgreSQL rollback, concurrent-order and same-key replay cases passed. |
| F2-A02 | safe receipt/schema/audit redaction | unit/API response scans | implemented-local: safe transport/read-model regression is included in the fresh `477 passed, 17 skipped` backend run. |
...
| F2-A06 | all seven aggregation and nested graph bounds | unit tests + PostgreSQL | implemented-local: unit tests cover nested/depth/cycle behavior; the dedicated authorised PostgreSQL projection case evaluates `values`, `count`, `count_distinct`, `sum`, `average`, `min`, `max` together against one safe view record. |
| F2-A07 | incoming-link/dependent-field guards | service and PostgreSQL tests | implemented-local: service guard coverage and one authorised PostgreSQL case prove the incoming-record, relation-field and target-field dependency conflicts. |
...
| F2-A10 | four-width actual Mini App QA | local fixture + browser screenshots + console scan | implemented-local: primary and negative-state fixture runs cover 1440/1280/430/390. The final 1440/390 pass verifies all seven aggregate values, safe relation labels, direct picker remove/reselect behavior and the current-record exclusion (`Current record` candidate count `0`, `Other record` candidate count `1`, selected chip count `1`); local-origin console was `[]`. Evidence remains local-fixture only, not Telegram/staging/production proof. |
