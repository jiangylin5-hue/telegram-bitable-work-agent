...
| `npm.cmd test -- --run src/test/view-access-panel.test.tsx src/test/governance-write-api.test.ts src/test/governance-write-query.test.ts src/test/governance-write-workbench.test.tsx src/test/governance-write-app-flow.test.tsx` | `5 files / 23 tests passed` | typed parser/key boundary, no raw-detail rendering, typed 409 retention plus explicit field-policy reread, restricted-owner V1 selection, exact Base 404 cleanup, close-focus return, and delayed old-workspace role/field-policy outcomes `401/403/404/409` that cannot deny or repopulate the replacement workspace. |
...
| GW-A06 | `implemented-local` | typed parser/key, no-optimistic reread, immediate 401/403 boundary, exact Base 404 cleanup, component 409 retention with explicit reread and close-focus return are covered. Delayed role and field-policy `401/403/404/409` replacement-workspace matrices are covered. |
