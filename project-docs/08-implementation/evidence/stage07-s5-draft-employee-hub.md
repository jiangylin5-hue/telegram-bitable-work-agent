...
| --- | --- | --- |
| `pytest tests/unit/test_stage07_draft_employee_hub_api.py tests/integration/test_stage07_draft_employee_hub_postgres.py -q` | `17 passed` | safe citation projection, cross-Base and post-selection hidden-view denial before runtime, pending-only newest-first keyset queue, confirm replay, reject no-record-write, concurrent terminal lock/loser-ledger rollback, and fresh revoked-field reread that suppresses the diff/value and disables confirmation |
