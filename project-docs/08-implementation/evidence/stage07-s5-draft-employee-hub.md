...
| fixed invocation controls | `summarize` requires current Base/view; `draft_update` remains disabled without an open record and submits a fresh idempotency key when enabled; the adapter rechecks the current visible View/record intersection before reserving that key or calling runtime | intent cannot be arbitrary; a result draft pointer is reread through the existing safe draft route |
...
| --- | --- | --- |
| `pytest tests/unit/test_stage07_draft_employee_hub_api.py tests/integration/test_stage07_draft_employee_hub_postgres.py -q` | `18 passed` | safe citation projection, cross-Base/post-selection hidden-View and record-filtered-out-of-current-View denial before runtime or draft-invocation reservation, pending-only newest-first keyset queue, confirm replay, reject no-record-write, concurrent terminal lock/loser-ledger rollback, and fresh revoked-field reread that suppresses the diff/value and disables confirmation |
