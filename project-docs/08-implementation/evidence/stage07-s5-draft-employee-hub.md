...
| --- | --- | --- |
| `pytest tests/unit/test_stage07_draft_employee_hub_api.py tests/integration/test_stage07_draft_employee_hub_postgres.py -q` | `15 passed` | safe citation projection/cross-Base denial, pending-only newest-first keyset queue, confirm replay, reject no-record-write and concurrent terminal lock/loser-ledger rollback on real local PostgreSQL |
