...
| --- | --- | --- |
| `pytest tests/unit/test_stage07_draft_employee_hub_api.py tests/integration/test_stage07_draft_employee_hub_postgres.py -q` | `14 passed` | safe citation projection/cross-Base denial, confirm replay, reject no-record-write and concurrent terminal lock/loser-ledger rollback on real local PostgreSQL |
| `npm.cmd test -- --run src/test/draft-employee-api.test.ts src/test/draft-employee-hub.test.tsx src/test/draft-employee-query.test.ts src/test/draft-employee-app-flow.test.tsx` | `4 files / 16 tests` | strict transport, current-Canvas summary body, disabled no-record draft control, fresh draft idempotency key, stale Canvas-result discard, existing safe queue and terminal reread flows |
...
- Pending-queue `EXPLAIN (ANALYZE, BUFFERS)` decision; optional partial index remains deferred until that documented gate is met.
