...
- Current Progress: V1-1 through V1-9 local durability, typed command/read-schema, canonicalization, ACL/versioned mutation, Grid query, safe HTTP, real database and transport foundations are implemented: `PlatformView` ownership/scope/version, `ViewMemberGrant`, migration `20260711_0022`, narrow UoW methods, strict presentation commands, server-composed safe projection, idempotent private initialization, effective owner/editor/viewer access, atomic grants, canonical filter/group/stable-sort execution before cursor pagination, five fixed-code safe routes, local PostgreSQL default/rollback/hidden-field/index-plan proof, typed Mini App safe transport/key/error helpers, and reusable V1 Builder/Access/Query panels. The panels consume only callback-provided safe projections, include `field_id` for F2 relation candidate reuse and select-only `filter_values`, but they are not yet wired into the App/BaseCanvas authoritative reread lifecycle or browser acceptance.
...
| --- | --- | --- | --- |
| V1-A01 | safe private initialization/replay/rollback | unit/API + real PostgreSQL | implemented-local: local API proves `201` create and `200` same-key safe replay; PostgreSQL proves post-initialization route failure rolls back view/audit/idempotency |
| V1-A02 | scope/owner/member ACL intersection | service/API negative tests + PostgreSQL unique grants | partial-local: resolved ACL and owner-only atomic grants have unit/API plus unique-grant evidence; Task 7 retains broader real-database denial proof |
| V1-A03 | owner/editor/viewer mutation separation | service/API tests | partial-local: API proves editor PATCH, owner-only members and viewer `403`; no Mini App proof |
...
| V1-A05 | typed configuration validation and safe projection | unit/API response scans + local PostgreSQL | partial-local: strict unknown-field rejection and safe context/builder/receipt scans pass locally; a real PostgreSQL Builder-context route test proves safe `field_id`/select `filter_values` projection without raw options/policy/internal target data |
...
| V1-A08 | Kanban/Calendar/Form configuration validation | unit/API + panel tests | partial-local: strict canonical unit coverage and direct type-specific panel controls exist; route/UI matrix remains pending |
| V1-A09 | protected query/cancellation/error containment | Mini App component/application tests | partial-local: typed V1 query keys and fixed error mapping are unit-tested; direct panels consume that mapper and enforce the owner/editor/viewer boundary, while panel failure/pending and component lifecycle/scope cancellation coverage remain pending Task 11 |
