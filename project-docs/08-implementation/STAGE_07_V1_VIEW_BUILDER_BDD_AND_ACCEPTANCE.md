...
| V1-A02 | scope/owner/member ACL intersection | service/API negative tests + local PostgreSQL | partial-local: resolved ACL and owner-only atomic grants have unit/API plus unique-grant evidence; real PostgreSQL now proves Base-list and direct-presentation denial before a grant and safe viewer summary/read after it |
| V1-A03 | owner/editor/viewer mutation separation | service/API + Mini App panel tests | partial-local: API proves editor PATCH, owner-only members and viewer `403`; direct Mini App panel tests hide grant controls for editor/viewer, but four-width Browser proof remains pending |
