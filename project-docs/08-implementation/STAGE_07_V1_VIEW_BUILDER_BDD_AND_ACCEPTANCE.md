...
| V1-A02 | scope/owner/member ACL intersection | service/API negative tests + local PostgreSQL | partial-local: resolved ACL and owner-only atomic grants have unit/API plus unique-grant evidence; PostgreSQL proves Base-list/direct-presentation denial before a grant and safe viewer summary/read after it; V1-13 Browser observed the owner/editor/viewer surface but cannot prove real-backend underlying Base/Table/Field intersection |
| V1-A03 | owner/editor/viewer mutation separation | service/API + Mini App panel tests | implemented-local: API proves editor PATCH, owner-only members and viewer `403`; component tests and V1-13 Browser prove owner member controls, editor save without member controls, and viewer read-only surface |
