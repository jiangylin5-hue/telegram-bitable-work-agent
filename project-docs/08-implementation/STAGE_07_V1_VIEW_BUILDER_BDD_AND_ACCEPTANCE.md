...
- Current Progress: V1-1 through V1-13 local durability, typed command/read-schema, canonicalization, ACL/versioned mutation, Grid query, safe HTTP, real database and transport foundations are implemented. Task 12 supplied the four-width core matrix; V1-13 adds owner/editor/viewer actual UI separation, disabled 13th filter/4th-sort entries, relation/lookup group exclusion, F2 safe relation-candidate label, editor save and stale-`409` canonical reread. V1-14/15 real FastAPI plus disposable PostgreSQL Browser passes prove role/intersection projection and one owner Record Detail relation edit: the safe candidate label is selected, existing versioned PATCH moves version `1` to `2`, and Canvas/Detail reread the relation plus numeric lookup value. Page console was `[]` and all temporary runtime material was removed. This remains partial Browser evidence: it does not prove a real stale-version/type-invalid matrix, every invalid typed configuration state or all-width real-backend coverage. The V1 Canvas receives server-filtered V1 summary markers, safe presentation and records only; save/create rereads list, Builder and record window. Presentation/member `409` clears incompatible drafts, rereads canonical protected state and emits only fixed local feedback; `422`/`5xx` retains a safe local draft without implicit retry.
...
And a visible field may expose its safe `field_id` only to bind existing F2 relation candidate selection, while a select-like field may expose only its validated `filter_values` string array for the typed chooser; it never exposes a raw `options` object, option metadata or choices for a hidden field.
And the ordinary Base view list omits a V1 private/restricted summary when that same caller has no effective V1 read access; it never turns an inaccessible name into a disabled tab or an existence-revealing error.
And an emitted V1 summary has only its documented safe scope/access/default markers, allowing a refreshed Canvas to select the Builder path without probing a legacy raw view.
And a direct legacy presentation-read URL cannot bypass this omission: V1 access is resolved before the legacy presentation path, while a permitted V1 reader receives only the same safe normalized presentation shape.
...
| --- | --- | --- | --- |
| V1-A01 | safe private initialization/replay/rollback | unit/API + real PostgreSQL | implemented-local: local API proves `201` create and `200` same-key safe replay; PostgreSQL proves post-initialization route failure rolls back view/audit/idempotency; Browser observed private creation |
| V1-A02 | scope/owner/member ACL intersection | service/API negative tests + local PostgreSQL | partial-local: resolved ACL and owner-only atomic grants have unit/API plus unique-grant evidence; PostgreSQL proves Base-list/direct-presentation denial before a grant and safe viewer summary/read after it; V1-14 real Browser proves the allowed Base/Table/Field intersection and hidden-field omission, but not the remaining denied-screen case |
| V1-A03 | owner/editor/viewer mutation separation | service/API + Mini App panel tests | implemented-local: API proves editor PATCH, owner-only members and viewer `403`; V1-14 real Browser proves owner member controls, editor save without member controls and repaired viewer read-only Canvas entry points; existing backend remains mutation-authoritative |
| V1-A04 | existing default Grid invariant | migration + integration tests | implemented-local: migration and PostgreSQL reject a second default while V1 private creation stays non-default |
| V1-A05 | typed configuration validation and safe projection | unit/API response scans + local PostgreSQL | partial-local: strict unknown-field rejection and safe context/builder/receipt scans pass locally; a PostgreSQL Builder-context route test proves safe `field_id`/select `filter_values` projection without raw options/policy/internal target data; V1-14 real Browser proves visible server projection and hidden `Internal` omission, but not every invalid payload |
| V1-A06 | server filter/sort/group before pagination | unit/integration + real PostgreSQL + Mini App lifecycle | implemented-local: Grid filter/group/sort-before-pagination and page-local safe group metadata are proven locally; Browser observed the server-authored summary and returned order without client query execution |
| V1-A07 | F2 relation/numeric lookup eligibility | unit/API + Picker contract tests | partial-local: canonical service eligibility is covered; V1-13 Browser changed a filter to relation, rendered the existing F2 picker and exposed only safe candidate label `Permitted linked record` without ID. V1-15 real Browser selected `Editable account` in Record Detail, committed the existing versioned PATCH from version `1` to `2`, then reread the safe relation and numeric `sum` lookup `7`. Numeric lookup filter mutation and invalid/stale paths remain unaccepted. |
| V1-A08 | Kanban/Calendar/Form configuration validation | unit/API + panel/renderer tests | partial-local: strict canonical unit coverage and type-specific panel controls exist; Browser observed private Kanban, Calendar and Form rendering, but not every type-specific invalid state |
| V1-A09 | protected query/cancellation/error containment | Mini App component/application tests | implemented-local: typed keys/fixed error mapping, authoritative rereads, safe `422` retention and focus-return tests pass; Browser observed `409` canonical reread with raw detail suppressed and 390px focus return; V1-13 repeats this actual conflict path as editor |
| V1-A10 | four-width actual Browser matrix | disposable local fixture + console scan | partial-local: 1440 Grid, 1280 Kanban, 430 Calendar/access sheet and 390 Form/focus plus final console `[]` were observed; V1-13 adds role and safe negative UI at desktop, and V1-14 adds desktop real-backend role/intersection evidence. All-width real backend, all type-invalid/F2 and complete role matrix remain unaccepted |
