...
- Current Progress: V1-1 through V1-12 local durability, typed command/read-schema, canonicalization, ACL/versioned mutation, Grid query, safe HTTP, real database and transport foundations are implemented. Task 12 added a disposable-fixture Browser run at 1440/1280/430/390: private Grid with server-authored filter/sort/group summary and owner grant, Kanban, Calendar mobile access-sheet, Form focus return and a raw-detail-suppressed `409` canonical reread; final console was `[]` and all temporary runtime material was removed. This is partial Browser evidence, not a real-backend or all-role/negative-state claim. The V1 Canvas receives server-filtered V1 summary markers, safe presentation and records only; save/create rereads list, Builder and record window. Presentation/member `409` clears incompatible drafts, rereads canonical protected state and emits only fixed local feedback; `422`/`5xx` retains a safe local draft without implicit retry.
...
| V1-A02 | scope/owner/member ACL intersection | service/API negative tests + local PostgreSQL | partial-local: resolved ACL and owner-only atomic grants have unit/API plus unique-grant evidence; PostgreSQL proves Base-list/direct-presentation denial before a grant and safe viewer summary/read after it; Browser observed only the owner grant action, not identity switching |
| V1-A03 | owner/editor/viewer mutation separation | service/API + Mini App panel tests | partial-local: API proves editor PATCH, owner-only members and viewer `403`; direct Mini App panel tests hide grant controls for editor/viewer, but this Browser fixture was owner-only |
...
| V1-A07 | F2 relation/numeric lookup eligibility | unit/API + Picker contract tests | partial-local: canonical service eligibility is covered; the direct Builder panel reuses the existing F2 candidate picker through safe `field_id`; F2 was not re-run inside this V1 Browser fixture |
...
| V1-A09 | protected query/cancellation/error containment | Mini App component/application tests | implemented-local: typed keys/fixed error mapping, authoritative rereads, safe `422` retention and focus-return tests pass; Browser observed `409` canonical reread with raw detail suppressed and 390px focus return |
| V1-A10 | four-width actual Browser matrix | disposable local fixture + console scan | partial-local: 1440 Grid, 1280 Kanban, 430 Calendar/access sheet and 390 Form/focus plus final console `[]` were observed; multi-role/invalid/F2/real-backend Browser flows remain unaccepted |
