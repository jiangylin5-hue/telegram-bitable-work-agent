...
| Fail closed on denied/revoked/expired session | SDD §3/8; BDD 12 | `partial-local` | Bootstrap/Home, Base-open and saved-view reads consume TanStack cancellation signals; 401 removes all Stage07 query state and 403 removes the affected workspace scope before a denied state; helper/application tests prove scope removal and stale Base/view protection | Record-detail and cursor-continuation reads plus a real expiry/revocation integration test remain unimplemented. |
