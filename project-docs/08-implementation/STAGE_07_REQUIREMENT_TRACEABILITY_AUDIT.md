...
| V1 Saved View Builder | Source §5; V1 Design; V1 BDD/SDD/Work Surface/Complex Index; V1 TDD Plan | `partial-local` | User-approved V1 work now has migration `20260711_0022`, model owner/scope/version state, `ViewMemberGrant`/narrow UoW operations, strict discriminated typed command/safe-read schemas, canonical/safe projection and versioned ACL mutation. Focused V1-1 evidence is `17 passed`; V1-2 schema/API regression is `11 passed`; V1-3 canonical/F2 regression is `43 passed`; V1-4 disposable PostgreSQL is `4 passed` for migration/replay/unique/concurrency; full backend is `499 passed, 17 skipped`; Alembic has one head. The existing default-Grid partial index is not changed. Optional non-unique access/list indexes are deferred pending Task 7 PostgreSQL `EXPLAIN` evidence. Legacy Stage06 raw view config/policy route remains unsuitable for Mini App use. | Implement server record-query execution, HTTP routes, final broader PostgreSQL/security proof, Mini App and four-width Browser acceptance. No V1 API or client behavior is accepted yet. |
