...
| Workspace switch removes old protected model | SDD §3; App Shell module | `partial-local` | `protectedQuery.ts` has user/workspace-prefixed keys and cancellation/removal; `App.tsx` clears the old scope before target Home and invalidates a Base-open generation; an application test proves delayed old Base tables/views cannot restore the previous canvas after a workspace switch | Saved-view, record-detail and cursor-continuation reads remain outside this boundary; full revocation/expiry coverage remains incomplete. |
...
| Authorized Base/table/view navigation | API Contract §4; Bitable module | `implemented-local` | approved navigation endpoints; initial Base-open reads are protected-query keyed by verified user/workspace; route/application tests and browser Grid path | Must add Base/table selection beyond the initial table and migrate saved-view selection separately. |
...
| Cursor-safe paging | Plan Package 2; Acceptance checklist | `implemented-local` | `api.viewRecords(viewId, cursor)`, BaseCanvas load-more control, component/application tests and desktop/390px browser QA | Cursor comes only from the server response; pages are deduplicated by record ID. The initial window is query-backed on Base open, while follow-up page state remains a separate migration slice. |
...
| Direct version-aware scalar edit | SDD §5; Plan Package 2 | `partial-local` | `RecordDetail.tsx`, `PATCH /records/{id}`, component/application conflict tests and browser QA | A 409 now refetches the permitted detail and current saved-view window before allowing a retry. The Base-open initial reads use protected queries, but record/conflict mutation refresh has no targeted query invalidation yet. Complex typed fields remain read-only. |
...
| Fail closed on denied/revoked/expired session | SDD §3/8; BDD 12 | `partial-local` | Bootstrap/Home and the initial Base-open dependency tree consume TanStack cancellation signals; 401 removes all Stage07 query state and 403 removes the affected workspace scope before a denied state; helper/application tests prove scope removal and stale Base protection | Saved-view, record-detail and cursor-continuation reads plus a real expiry/revocation integration test remain unimplemented. |
...
2. **Approved architecture implementation in progress:** Technical Decision 001 now has a memory-only QueryClient, user/workspace key contract, cancellation/removal helpers, bootstrap/Home migration and initial Base-open migration. Migrate saved-view, record-detail, cursor continuation and mutation refresh in independently tested slices; do not expand its scope into persistence, governance or Package 4.
