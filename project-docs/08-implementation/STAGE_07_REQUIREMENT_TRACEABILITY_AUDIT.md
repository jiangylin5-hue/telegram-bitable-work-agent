...
2. **Approved architecture implementation:** Technical Decision 001 now authorizes the memory-only protected TanStack Query boundary. Begin with the document's red security tests before adding the dependency; do not expand its scope into persistence, governance or Package 4.
