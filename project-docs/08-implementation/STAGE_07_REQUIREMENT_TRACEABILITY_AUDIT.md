...
| Role/permission editor | TD004; Governance Write BDD/SDD | `proposed-awaiting-approval` | complete S4 decision/specification defines fixed-role member command, field-policy replacement and existing V1 grant reuse | Requires explicit approval before revision migration, new actions/routes or client mutation code; browser must not reconstruct policy semantics. |
