...
| Workspace switch removes old protected model | SDD §3; App Shell module | `partial-local` | `protectedQuery.ts` has user/workspace-prefixed keys and cancellation/removal; `App.tsx` now clears the old Home scope before loading the target; frontend/browser switch checks prove old Home navigation disappears | Base/table/view/record reads have not yet migrated to the protected query boundary; full revocation/expiry coverage remains incomplete. |
...
| Fail closed on denied/revoked/expired session | SDD §3/8; BDD 12 | `partial-local` | Bootstrap/Home now use TanStack cancellation signals; 401 removes all Stage07 query state and 403 removes the affected workspace scope before a denied state; helper tests prove scope removal | Base/table/view/record reads and a real expiry/revocation integration test remain unimplemented. |
