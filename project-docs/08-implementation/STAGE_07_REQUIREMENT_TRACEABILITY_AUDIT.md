...
- Current Progress: 2026-07-11 audit reconciled after the bounded F1 Independent Field Builder and its retained direct-edit visual evidence. Package 1 and limited Package 2 paths have evidence, including P3 Base/Table initialization and F1 field initialization; the P3 and F1 disposable real-PostgreSQL integration commands remain unavailable locally. F1 duplicate-name and pending-dialog browser cases now have four-width evidence; delayed workspace/view replacement remains an application-level scope-isolation proof. F2 relation/lookup, additional-view Builder, imports, Package 3 governance and all Package 4 Digital Employee scope remain incomplete or contract-gated.
...
| F1 independent Field Builder | Source §5; F1 Design; F1 Plan; Bitable module | `partial-local` | safe `SafeTableField` Canvas projection; `POST /tables/{table_id}/field-initializations`; server-generated key/order/default policy; field-row lock, idempotency and sanitized audit; choice-aware create/direct edit; protected re-read/cache cleanup; backend and frontend F1 tests; four-width disposable Browser QA; retained `artifacts/stage07/f1-direct-edit-success-1440.png`; duplicate-name allowlist/pending-dialog browser runs at 1440px/1280px/430px/390px; `mini-app/design-qa.md` | The exact three real-PostgreSQL cases (rollback, same-key replay, distinct-key concurrent order/view append) remain skipped without `STAGE06_LOCAL_DATABASE_URL`. A corrected fixture proves direct-edit success visually. At all four widths the duplicate response retains its field value, maps only the allowlisted code and suppresses the server message; the pending dialog locks create/close/cancel. Delayed workspace replacement remains covered by the application scope-isolation test. F2/V1/import/governance/Package 4 remain out of scope. |
...
3. **P3 and F1 are implemented with database-evidence gaps:** run their exact disposable real-PostgreSQL rollback/concurrency suites only when an authorised `STAGE06_LOCAL_DATABASE_URL` is available. F1's direct-edit, duplicate-name feedback and pending-dialog lock now have four-width browser evidence; delayed workspace replacement remains an application-level scope-isolation proof. Do not treat the old fixture-only false negative as a production defect.
