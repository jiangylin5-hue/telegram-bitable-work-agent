...
2. **Approved architecture implementation in progress:** Technical Decision 001 now has a memory-only QueryClient, user/workspace key contract, cancellation/removal helpers, bootstrap/Home migration, Base/view/record reads and cursor continuation. Migrate mutation refresh in an independently tested slice; do not expand its scope into persistence, governance or Package 4.
