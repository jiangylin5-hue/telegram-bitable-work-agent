...
| F2 relation/lookup Builder | Source §5; F2 Design; F2 Plan; Bitable module | `approved-design-unimplemented` | User-approved `docs/superpowers/specs/2026-07-11-stage07-f2-relation-lookup-design.md`; detailed TDD plan `docs/superpowers/plans/2026-07-11-stage07-f2-relation-lookup-implementation.md`; existing Stage06 relation/lookup primitives only | The proposed safe browser API/read-model/permission-enforcement changes still need plan review and separate explicit implementation authorization. No F2 runtime code/evidence exists yet. |
...
4. **F2 design gate closed; implementation gate remains:** F2 relation/lookup has a user-approved specification and detailed plan. It must not begin until the user approves the plan and separately authorizes its proposed API/read-model/permission-enforcement changes. V1 additional views and import/template still each need their own safe browser contract and documented interaction design. Governance and field-filtered draft detail remain separate specification work.
