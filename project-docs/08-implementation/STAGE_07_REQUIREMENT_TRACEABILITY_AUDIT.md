...
| F1 independent Field Builder | Source §5; F1 Design; F1 Plan; Bitable module | `partial-local` | safe `SafeTableField` Canvas projection; `POST /tables/{table_id}/field-initializations`; server-generated key/order/default policy; field-row lock, idempotency and sanitized audit; choice-aware create/direct edit; protected re-read/cache cleanup; backend and frontend F1 tests; four-width disposable Browser QA; `mini-app/design-qa.md` | The exact three real-PostgreSQL cases (rollback, same-key replay, distinct-key concurrent order/view append) remain skipped without `STAGE06_LOCAL_DATABASE_URL`. Browser evidence shows record creation with allowed choices, but the original full fixture returned its read response before its test-only `PATCH` branch, so a completed direct-edit *visual* success view needs a corrected disposable-fixture rerun. This is not a production-path failure; automated direct-edit tests pass. F2/V1/import/governance/Package 4 remain out of scope. |
