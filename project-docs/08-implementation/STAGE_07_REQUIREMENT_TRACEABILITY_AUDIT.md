...
| Direct version-aware scalar edit | SDD §5; Plan Package 2 | `partial-local` | `RecordDetail.tsx`, protected record-detail opening, `PATCH /records/{id}`, component/application conflict tests and browser QA | A 409 now refetches the permitted detail and current saved-view window before allowing a retry. Record opening uses a protected query, but record/conflict mutation refresh has no targeted query invalidation yet. Complex typed fields remain read-only. |
...
| Fail closed on denied/revoked/expired session | SDD §3/8; BDD 12 | `partial-local` | Bootstrap/Home, Base-open, saved-view and record-detail reads consume TanStack cancellation signals; 401 removes all Stage07 query state and 403 removes the affected workspace scope before a denied state; helper/application tests prove scope removal and stale Base/view/record protection | Cursor-continuation reads plus a real expiry/revocation integration test remain unimplemented. |
