...
- Current Progress: 2026-07-11 audit reconciled after the bounded F1 Independent Field Builder and its retained direct-edit visual evidence. Package 1 and limited Package 2 paths have evidence, including P3 Base/Table initialization and F1 field initialization; the P3 and F1 disposable real-PostgreSQL integration commands remain unavailable locally. F1 still lacks explicit duplicate-name and interrupted workspace/view field-initialization browser cases. F2 relation/lookup, additional-view Builder, imports, Package 3 governance and all Package 4 Digital Employee scope remain incomplete or contract-gated.
...
| --- | --- | --- |
| Frontend unit/integration tests | `npm.cmd test -- --run` in `mini-app`: 13 files / 55 tests passed | uses mocked server responses; does not prove backend authorization. |
| Frontend production build | `npm.cmd run build`: passed | proves TypeScript/Vite build only. |
| Full backend regression | `python -m pytest -q` in `backend`: 432 passed, 25 skipped | 17 historical online PostgreSQL skips require `STAGE02_ONLINE_DATABASE_URL`; five Stage06 local PostgreSQL security skips and three F1 disposable PostgreSQL cases require `STAGE06_LOCAL_DATABASE_URL`. |
...
| Browser mobile QA | F1 field creation opened from the nonempty-table trigger at `390x844`; the sheet, retry/denial states and accessibility path were checked | duplicate-name and interrupted workspace/view field-init browser cases are still missing. |
...
| F1 independent Field Builder | Source §5; F1 Design; F1 Plan; Bitable module | `partial-local` | safe `SafeTableField` Canvas projection; `POST /tables/{table_id}/field-initializations`; server-generated key/order/default policy; field-row lock, idempotency and sanitized audit; choice-aware create/direct edit; protected re-read/cache cleanup; backend and frontend F1 tests; four-width disposable Browser QA; retained `artifacts/stage07/f1-direct-edit-success-1440.png`; 1440px duplicate-name allowlist/pending-dialog browser run; `mini-app/design-qa.md` | The exact three real-PostgreSQL cases (rollback, same-key replay, distinct-key concurrent order/view append) remain skipped without `STAGE06_LOCAL_DATABASE_URL`. A corrected fixture proves direct-edit success visually. The 1440px duplicate response retains its field value, maps only the allowlisted code and suppresses the server message; the pending dialog locks create/close/cancel. Repeat these two states at 1280px/430px/390px. Delayed workspace replacement remains covered by the application scope-isolation test. F2/V1/import/governance/Package 4 remain out of scope. |
...
| Visual QA at 1440, 1280, 430 and 390 | Test Plan §2; Acceptance checklist | `partial-local` | F1 disposable Browser QA exercised 1440/1280 desktop and 430/390 mobile; Workspace Ledger and a rendered F1 desktop state were reviewed together; blank validation, 503 retry, 409 lock, 403 denial, the repaired 390px mobile Add Field entry and direct-edit success were observed with zero relevant console warnings/errors. The direct-edit visual is retained at `artifacts/stage07/f1-direct-edit-success-1440.png`. | This is F1-scoped visual evidence only. It does not claim full Workspace Ledger/right-assistant-rail parity or whole-stage design parity. Duplicate-name server feedback and an in-flight field request interrupted by workspace/view replacement still need matching browser evidence. |
...
3. **P3 and F1 are implemented with database-evidence gaps:** run their exact disposable real-PostgreSQL rollback/concurrency suites only when an authorised `STAGE06_LOCAL_DATABASE_URL` is available. F1's corrected direct-edit fixture already has a retained success artifact; the remaining browser evidence is duplicate-name server feedback and an in-flight request interrupted by workspace/view replacement. Do not treat the old fixture-only false negative as a production defect.
