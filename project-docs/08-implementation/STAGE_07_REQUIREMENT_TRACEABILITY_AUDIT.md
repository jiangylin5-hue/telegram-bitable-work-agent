...
- Current Progress: 2026-07-11 audit reconciled after real local PostgreSQL verification of the bounded P3 Base/Table Builder and F1 Independent Field Builder, plus an approved-TD001 direct-record mutation safety repair. The disposable migration smoke reached `20260710_0021`; three P3 and three F1 rollback/replay/concurrency/default-view cases passed. The latest full backend suite passed `463` tests with `27` skips; the Mini App suite passed `18` files / `92` tests and production build. F1 duplicate-name and pending-dialog browser cases have four-width evidence. F2 relation/lookup has approved design/plan plus detailed BDD/SDD/module/index documents; backend and bounded frontend paths are locally implemented, and Browser evidence covers the four-width primary flow, desktop nested/replay/conflict/invalid/denied states, and mobile 430px conflict lock plus 390px candidate-denial boundary with a clean local-origin console. F2 PostgreSQL plus mobile replay/invalid evidence still lack proof; no F2 acceptance is claimed. V1 additional-view Builder, imports, Package 3 governance and all Package 4 Digital Employee scope remain incomplete or contract-gated.
...
| --- | --- | --- |
| Frontend unit/integration tests | `npm.cmd test -- --run` in `mini-app`: 18 files / 92 tests passed | uses mocked server responses; does not prove backend authorization. |
| Frontend production build | `npm.cmd run build`: passed | proves TypeScript/Vite build only. |
| Full backend regression | `python -m pytest -q` in `backend`: 463 passed, 27 skipped | F2 PostgreSQL remains skipped without `STAGE06_LOCAL_DATABASE_URL`; the run is local evidence, not staging/production. |
...
| F2 relation/lookup Builder | Source §5; F2 Design; F2 Plan; F2 BDD/SDD/Work Surface/Complex Index | `partial-local` | User-approved design/plan; detailed F2 BDD/SDD/module/index; local backend commits through `fe0b44f`; protected candidate/Pickers and create/direct-edit commits `09de7fc`/`a38b912`; safe Detail/Canvas rendering `0871dee`; exact candidate cleanup `3a8c2bc`/`8260154`; Builder and race repairs `9c25172`, `919cdc7`, `b22ee19`; App flow evidence `31b8784`; final Picker rejection/cursor-retry repair; Mini App suite `18` files / `92` tests and build pass; task reviews approved; primary Browser flow passed at 1440/1280/430/390, desktop nested/replay/conflict/invalid/denied states passed, and mobile 430px conflict-lock plus 390px candidate-403/global-denied paths passed with local-origin console `[]`; full backend `463 passed, 27 skipped`. | F2 PostgreSQL cases are environment-gated, and mobile replay/allowlisted-invalid evidence plus final requirement audit remain open. No F2 acceptance or production claim. |
...
4. **F2 local implementation is in progress, not accepted:** F2 has approved design/plan and separate BDD/SDD/module/index sources. Backend transport/initialization/safe read-write/evaluation and the bounded frontend Picker, Detail/Canvas and Builder paths have local evidence. Four-width primary and desktop negative-state Browser evidence is recorded, followed by a 430px conflict lock and 390px candidate-denial mobile proof. Next work is the authorised disposable PostgreSQL matrix, mobile replay/allowlisted-invalid evidence and the requirement-by-requirement acceptance audit. No F2 acceptance is implied. V1 additional views and import/template still each need their own safe browser contract and documented interaction design. Governance and field-filtered draft detail remain separate specification work.
