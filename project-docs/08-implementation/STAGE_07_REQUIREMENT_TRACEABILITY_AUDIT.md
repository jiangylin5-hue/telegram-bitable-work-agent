...
- Current Progress: P3/F1/F2 remain bounded `implemented-local` evidence. The user has approved V1 Saved View Builder's comprehensive design and authorized its implementation. V1-1 through V1-9 is `partial-local`: migration `20260711_0022`, durable owner/scope/version state, unique grant rows/UoW primitives, strict discriminated command/safe-read schemas, canonical readable-field/F2-safe projection, versioned ACL mutation, server filter/group/sort pagination, five safe HTTP routes, disposable PostgreSQL default/rollback/hidden-field/index-plan evidence, typed Mini App transport/protected keys/error mapping, and direct-tested Builder/Access/Query panel components exist. App lifecycle/capability rendering and Browser behavior remain unaccepted. Imports, Package 3 governance and all Package 4 Digital Employee scope remain incomplete or contract-gated.
...
| --- | --- | --- |
| Frontend unit/integration tests | `npm.cmd test -- --run` in `mini-app`: 18 files / 93 tests passed | uses mocked server responses; does not prove backend authorization. |
...
| Full backend regression | `python -m pytest -q` in `backend`: 477 passed, 17 skipped | skips are only historical Stage02 online smoke tests without `STAGE02_ONLINE_DATABASE_URL`; the run is local evidence, not staging/production. |
...
| V1 Saved View Builder | Source §5; V1 Design; V1 BDD/SDD/Work Surface/Complex Index; V1 TDD Plan | `partial-local` | User-approved V1 work now has migration `20260711_0022`, model owner/scope/version state, `ViewMemberGrant`/narrow UoW operations, strict discriminated typed command/safe-read schemas, canonical/safe projection, versioned ACL mutation, V1 Grid server filter/group/stable-sort execution before cursor pagination with page-local safe group metadata, five safe HTTP routes, local PostgreSQL default/rollback/hidden-field/index-plan evidence plus real safe `field_id`/select `filter_values` projection, typed Mini App safe transport/protected keys/fixed errors, and direct-tested Builder/Access/Query panel components. Focused V1-1 evidence is `17 passed`; V1-2 schema/API regression is `11 passed`; V1-3 canonical/F2 regression is `43 passed`; V1-4 disposable PostgreSQL is `4 passed` for migration/replay/unique/concurrency; V1-5 focused unit/PostgreSQL query evidence is `8 passed`; V1-6 API/legacy regression is `9 passed`; V1-7/9 PostgreSQL suite is `10 passed`; V1-8 transport tests are `15 passed`; V1-9 panel/F1/F2 regression is `13 passed`, full Mini App is `21 files / 102 tests` plus production build; backend full regression has not yet been rerun after V1-9. Alembic has one head. The existing default-Grid partial index is not changed. Optional non-unique access/list indexes are deferred by retained `EXPLAIN` evidence. Legacy Stage06 raw view config/policy route remains unsuitable for Mini App use. | Connect the panels to authoritative App/BaseCanvas selected-view lifecycle and then run four-width Browser acceptance. No V1 Builder lifecycle or browser behavior is accepted yet. |
...
4. **F2 bounded implementation is complete locally:** F2-A01 through F2-A10 have approved design/plan, detailed BDD/SDD/module/index, safe backend/frontend implementation, Mini App `18` files / `93` tests, real disposable PostgreSQL `6 passed`, full backend `477 passed, 17 historical Stage02 skips`, and recorded four-width Browser evidence. This is not Telegram, staging, production or Stage07 acceptance. Next work is separately scoped V1 additional views or import/template only after their safe browser contracts and documented interaction designs are approved. Governance and field-filtered draft detail remain separate specification work.
