...
| F2 relation/lookup Builder | Source §5; F2 Design; F2 Plan; F2 BDD/SDD/Work Surface/Complex Index | `partial-local` | User-approved design/plan; detailed F2 BDD/SDD/module/index; local commits `37b1e95` safe transport and `c3688da` atomic relation initializer | Candidate picker, record write rechecks, lookup evaluator/guards, PostgreSQL, frontend and browser evidence are absent. Lookup work is paused/uncommitted for document reconciliation; no F2 acceptance or production claim. |
...
4. **F2 documentation reconciliation gate:** F2 has approved design/plan and separate BDD/SDD/module/index sources. Only safe transport and atomic relation initialization have local commits. Do not resume lookup, candidate, record-write, PostgreSQL or frontend work until the user reviews this detailed documentation package; no F2 acceptance is implied. V1 additional views and import/template still each need their own safe browser contract and documented interaction design. Governance and field-filtered draft detail remain separate specification work.
