...
| Field and additional View Builder | Source §5; Plan Package 2; Bitable module | `existing-contract-unimplemented` | Stage06 primitives create field/view resources, but their raw config/policy responses are unsuitable for routine client navigation | No approved P3 expansion exists for field schema design, typed field creation/editing, additional view configuration or default-view switching. A later safe read/write interaction contract is required. |
...
4. **Further interaction/contract specification required before UI:** complex typed field editors; field/additional-view builder schema/read/edit model; import/template user flow; governance role/permission model; field-filtered draft detail.
