...
| Templates and import | Source §5; Package 2; Template/Import Design, BDD, SDD, Work Surface and Complex Index | `specified-awaiting-review` | Existing `stage06_templates.py`: list, install, create/read/commit import, save Base template; selected docs define safe metadata-only shelf, install reread, Base draft save, CSV/XLSX encoding, persisted preview, scalar mapping, explicit commit, error allowlist and protected cleanup. | No UI exists yet. User review of the full written specification is required before plan/code; no contract expansion, Base picker, storage/queue, template publication or recovery-list route is assumed. |
