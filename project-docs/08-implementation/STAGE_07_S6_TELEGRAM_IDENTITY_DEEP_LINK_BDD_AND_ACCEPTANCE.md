...
| S6-A07 | resolved target is reread and stale-safe | Mini App flow verifies resolver pointer causes Base/View/Record rereads before display; recovery focus plus target-reread `401`/`403` denied-generation regressions pass | partial-local; `404`/`409`/`422`/network/supersession matrix remains open |
