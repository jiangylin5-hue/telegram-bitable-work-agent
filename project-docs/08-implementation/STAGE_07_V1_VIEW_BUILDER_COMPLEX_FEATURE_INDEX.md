...
- Current Progress: V1-1 through V1-13 create the correctness-critical grant uniqueness constraint, strict typed command/read boundary, canonical safe query-shape projection, server query ordering, five safe routes, PostgreSQL/index evidence, protected client keys, exact rereads and Browser observations. V1-13 confirms owner/editor/viewer UI separation, bounded filter/sort/group entries, safe relation candidate label, editor save and fixed-detail-suppressed `409`; real-backend authorization intersection and every F2/type-invalid Browser state remain unaccepted. Task 7 measured the access query using existing `ix_stage06_views_table_id` and `uq_view_member_grants_view_user`; `ix_view_member_grants_user_status` and `ix_views_table_scope_status` remain explicitly deferred, not merely unimplemented.
