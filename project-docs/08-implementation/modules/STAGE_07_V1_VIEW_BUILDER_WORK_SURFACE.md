...
- Current Progress: V1-1 through V1-13 durable persistence, typed schema, canonicalization, ACL/versioned mutation, server-owned query ordering, five safe routes, PostgreSQL proof, safe transport, panels, Canvas reread and recovery are implemented. V1 Canvas replaces prior inert query buttons with authorized entry points, renders only server output and rereads canonical state after `409`. Disposable Browser evidence now observes four renderers plus owner/editor/viewer separation, bounded query controls, safe F2 relation label, editor save and conflict redaction. It does not replace real-backend authorization intersection, every invalid/F2 state or production proof.
...
Tasks 12--13 now have V1 Browser observations for private create, owner/editor/viewer UI separation, typed Grid filter/sort/group, one type-specific renderer at each of 1440/1280/430/390, relation candidate label, conflict containment and final console scan. Real-backend intersection, complete type-invalid/F2 states and every role at every viewport remain required before complete V1 Browser acceptance. Existing F1/F2 Browser artifacts still do not count as V1 evidence.
