...
| S6.2 persists an opaque raw link in legacy message/audit storage | forwarded bearer token or sensitive destination leakage | TD008 uses a typed one-to-one delivery extension; only Worker memory may hold token/URL and every persisted/loggable surface has a negative scan | before any S6.2 code |
| S6.2 retries after an uncertain Bot API result | duplicate external notification or accidental delivery amplification | durable `dispatch_reserved` before network; `delivery_unknown` revokes the pointer and forbids automatic retry | Worker fault-injection and PostgreSQL lock tests |
| Restricted-test target is a group/channel or multiple chat IDs | unintended audience expansion | human-attested private test chat plus exactly one runtime allowlist value equal to active source binding chat | per-environment authorization before smoke |
