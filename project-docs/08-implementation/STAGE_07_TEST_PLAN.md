...
- Still required before an F2 acceptance report: an authorised disposable PostgreSQL pass (the dedicated suite remains `2 skipped` without `STAGE06_LOCAL_DATABASE_URL`) and the final requirement-by-requirement acceptance audit, which must reconcile all BDD rows against their recorded evidence. Full backend regression is no longer an open F2 evidence item.
