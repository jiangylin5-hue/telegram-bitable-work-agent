...
- Current Progress: 2026-07-08 Test plan completed for Stage05 functional/staging acceptance. Local tests cover config, AgentRun evidence, router schema, workflow, child agents, service draft API, account inventory, confirmation/no-op, customer reply send, views, staging contract, scope guard, deployment config and runtime summary. Real staging tests have now run: migration head, real OpenRouter, allowlisted Telegram receipt, no-op evidence, account exception, additional three-message exercise and safety close. Remaining optional test gap is the 17 online PostgreSQL smoke tests skipped without `STAGE02_ONLINE_DATABASE_URL`.
