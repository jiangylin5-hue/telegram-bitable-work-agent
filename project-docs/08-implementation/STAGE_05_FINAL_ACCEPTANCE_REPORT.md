...
- Current Progress Update: 2026-07-08 Local real OpenRouter workflow passed after the entity-key prompt contract fix. Real OpenRouter returned schema-valid outputs for both a routed draft scenario and a default risk/manual-review scenario. The routed scenario created pending-confirmation recharge and customer-reply drafts with no provider execution or Telegram send; the risk scenario stayed in manual review with no side effects.
...
| Local real OpenRouter workflow gate | `STAGE_05_LOCAL_REAL_WORKFLOW_ENV.md`; `backend/scripts/stage05_local_real_workflow.py` passed two real OpenRouter scenarios. Routed evidence: `workflow_status=routed`, `intent_types=["recharge","customer_reply"]`, `service_drafts=["recharge:pending_confirmation","customer_reply:pending_confirmation"]`, `reply_text_present=true`, `provider_execution_allowed=false`, `send_request_created=false`. Risk evidence: `workflow_status=manual_review`, `intent_types=["recharge","bm_invite","customer_reply","account_status_exception"]`, no service drafts and no side effects. |
...
| --- | --- | --- | --- |
| R05-01 OpenRouter hallucinated intent/entities | locally mitigated; first real staging output still failed schema | Router schema, confidence/manual-review and invalid-output tests pass; router prompt contract now includes explicit top-level keys, allowed intent types, schema-valid example and entity-key guidance; local real workflow passed routed draft and risk/manual-review scenarios with real OpenRouter | Deployed worker still needs retry evidence after prompt-contract redeploy |
