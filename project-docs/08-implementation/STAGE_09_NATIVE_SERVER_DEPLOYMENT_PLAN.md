...
- Current Progress Update (2026-07-28, Stage10 r66 active): Public source, venv and static pointers now resolve to `stage09-p1-20260728-r66-conversation-routing`; production PostgreSQL is at the single Alembic head `20260728_0034`. API, background worker, outbox bridge, Agent outbox publisher, Agent tabular worker, dedicated Redis and Nginx are active. The sealed artifact, migration, service/data assets, Nginx SSE policy, static parity and bounded readiness gates passed. Real browser checks covered explicit skill selection, automatic table retrieval, pure greeting and greeting-plus-business routing; local/server JS and CSS hashes match exactly. Failed r62/r63 candidates and the isolated acceptance environment were removed; r64/r65 and the pre-r64 database backup remain as rollback evidence. Full sanitized evidence is `evidence/stage10-r66-public-deployment-and-ui-acceptance-2026-07-28.md`.

- Current Progress Update (2026-07-27, static parity enforcement local-ready): A sealed read-only `verify-static-artifact-parity.sh <artifact-id>` gate now derives the fixed source (`/opt/stage09-p1/releases/<id>`), venv (`/opt/stage09-p1/venv/<id>`) and public static (`/var/www/stage09-p1/<id>`) candidates and rejects link traversal, candidate-id mismatch, absent entry points, missing/invalid static manifest, hash drift and missing Vite-referenced local assets. It is wired into release-layout/release-assets contracts and has an isolated fixture suite. No server artifact was uploaded, no current link changed and no service/database/Telegram/LLM action occurred. This local-ready gate supersedes the earlier checklist-only static-parity wording; activation still requires an explicitly authorized deployment after a complete matching static artifact has been built.
...
1. **封存 artifact**：校验 commit、release checksum、Alembic 固定目标 `20260720_0032` 和 P1-A 预检；若 `mini-app/**` 有变更，必须先从同一候选构建完整静态包（`index.html`、引用 assets、`browser-handoff.html`、ID marker 和相对路径 hash manifest），再在切换前运行 `verify-static-artifact-parity.sh <artifact-id>`。它必须同时看到同 ID 的 source/venv/static，任何失败均不得切换三条 `current`。不在服务器执行 `git pull`，不改历史 Stage03 checkout。
