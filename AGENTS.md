...
- Current Progress: 2026-07-12 Stage06 backend-stage acceptance remains the baseline. Stage07 bounded Package 2 substages P3, F1 and the user-approved F2 relation/lookup slice are `implemented-local`. V1-1 through V1-12 are `partial-local`: durable typed/canonical/versioned/ACL/query/HTTP/PostgreSQL/transport/panel/Canvas recovery work is implemented, and an actual disposable-fixture Browser run observed Grid/Kanban/Calendar/Form at 1440/1280/430/390, owner editor grant, raw-detail-suppressed `409` canonical reread, 390px focus return and console `[]`. The fixture did not exercise multi-identity roles, every invalid/F2 Browser case or real-backend traffic; those remain unaccepted. The Base-list and direct-presentation paths resolve V1 ACL before emitting a summary or projection; only V1 safe scope/access/default markers supplement the legacy list shape. The measured access query uses existing indexes, so both optional non-unique V1 indexes remain explicitly deferred and no migration was added. Telegram identity, staging, production and Stage07 acceptance remain pending.
