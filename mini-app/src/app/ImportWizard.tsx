...
export function ImportWizard({ target, onCreatePreview, onCommit, onClose }: Props) {
  ...
  async function commit(event: FormEvent<HTMLFormElement>) {
    ...
    try { setReceipt(await onCommit(preview.id, { baseName: normalizedBaseName, tableName: normalizedTableName, tableKey: normalizedTableKey, fieldMapping: mapping })) } catch (caught) { setError(safeError(caught)) } finally { setPending(null) }
  }
