...
const importFieldTypes: Array<{ value: ImportScalarFieldType; label: string }> = [
  { value: 'text', label: '文本' },
...
const importFieldTypeValues = importFieldTypes.map((item) => item.value)
...
function safeError(error: unknown) {
  if (error instanceof ApiError) {
    if (error.status === 401) return '浏览器会话已失效，请从 Telegram 重新打开工作台后重试'
    if (error.status === 403) return '当前身份没有导入权限'
...
function validMapping(mapping: ImportMapping[]) {
  ...
  for (const item of mapping) {
    if (!item.targetKey || !importFieldTypeValues.includes(item.fieldType) || targets.has(item.targetKey)) return false
