...
export function BaseCanvas({ base, tables, views, table, view, schema, records, presentation, loadingMore, loadMoreError, serverQuerySummary, onBack, onOpenRecord, onSelectTable, onSelectView, onLoadMore, onCreateRecord, canManageSchema = false, canCreateViews = false, canManageViews = false, canCreateRecords = false, canManageDigitalEmployees = false, onCreateTable, onCreateField, onCreateView, onConfigureView, onSaveTemplate, onImportIntoBase, onOpenDraftHub, onOpenDigitalEmployeeManagement }: BaseCanvasProps) {
  ...
  return <main className="base-canvas" aria-label="Base 工作台">
    <header className="canvas-header"><button className="back-link" type="button" onClick={onBack}><ArrowLeft size={16} /> 工作区</button><span className="canvas-separator">/</span><h1>{base.name}</h1>{canManageSchema && (onSaveTemplate || onImportIntoBase) ? <div className="base-more-actions"><button className="icon-button" aria-label="更多 Base 操作" aria-expanded={moreOpen} type="button" onClick={() => setMoreOpen((open) => !open)}><MoreHorizontal size={19} /></button>{moreOpen ? <div className="base-more-menu">{onImportIntoBase ? <button type="button" onClick={() => { setMoreOpen(false); onImportIntoBase() }}>导入到当前 Base</button> : null}{onSaveTemplate ? <button type="button" onClick={() => { setMoreOpen(false); onSaveTemplate() }}>保存为模板</button> : null}</div> : null}</div> : null}</header>
    ...
    <div className="view-toolbar"><div role="tablist" aria-label="保存视图">{views.filter((item) => item.table_id === table.id).map((item) => <button role="tab" aria-selected={item.id === view.id} className={item.id === view.id ? 'view-tab active' : 'view-tab'} type="button" key={item.id} onClick={() => onSelectView(item.id)}>{item.name}</button>)}</div><div className="view-tools">{serverQuerySummary ? <output className="view-query-summary" aria-label="服务器查询摘要">{serverQuerySummary}</output> : null}{canManageDigitalEmployees && onOpenDigitalEmployeeManagement ? <button className="open-employee-management" type="button" onClick={(event) => onOpenDigitalEmployeeManagement(event.currentTarget)}>数字员工管理</button> : null}{onOpenDraftHub ? <button className="open-employee-hub" type="button" onClick={(event) => onOpenDraftHub(event.currentTarget)}>数字员工</button> : null}{canManageViews && onConfigureView ? <button className="configure-view-button" type="button" onClick={onConfigureView}>配置视图</button> : null}{canCreateViews && onCreateView ? <button className="create-view-button" type="button" onClick={onCreateView}>新建视图</button> : null}{canManageSchema && onCreateField ? <button className="add-field-button" type="button" onClick={onCreateField}>添加字段</button> : null}{canCreateRecords && schema.fields.length > 0 && onCreateRecord ? <button className="create-record-button" type="button" onClick={onCreateRecord}>新建记录</button> : null}</div></div>
