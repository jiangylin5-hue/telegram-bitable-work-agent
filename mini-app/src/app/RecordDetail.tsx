...
import { ApiError, type RecordDetail, type RelationCandidatePage, type TableSchema } from './api'
import { RelationChips, safeRelationCells } from './RelationChips'
...
const directFieldTypes = new Set(['text', 'status', 'single_select', 'multi_select', 'user', 'url', 'email', 'phone', 'date', 'number', 'checkbox', 'linked_record'])
...
export function RecordDetailPanel({ detail, schema, onClose, onSave, onConflict, loadRelationCandidates }: RecordDetailPanelProps) {
  ...
  async function save() {
    const changedValues = Object.fromEntries(Object.entries(values)
      .filter(([key, value]) => value !== current.values[key])
      .map(([key, value]) => [key, fields.find((field) => field.key === key)?.field_type === 'linked_record' ? safeRelationCells(value).map((item) => item.id) : value]))
