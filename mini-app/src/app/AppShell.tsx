...
function NavigationButton({ item, activeRoute, className, onPlanned }: { item: NavigationItem; activeRoute?: AppShellRoute; className: string; onPlanned?: (item: NavigationItem) => void }) {
  ...
  return <button className={`${className}${isActiveRoute ? ' active' : ''}${planned ? ' planned' : ''}`} type="button" aria-current={isActiveRoute ? 'page' : undefined} aria-expanded={item.controls ? item.expanded : undefined} aria-controls={item.controls} aria-label={label} title={label} data-nav-hint={hint} data-availability={item.availability} aria-disabled={!enabled ? 'true' : undefined} disabled={!enabled && !planned} onClick={enabled ? (event) => item.onClick?.(event.currentTarget) : planned ? () => onPlanned?.(item) : undefined}><Icon aria-hidden="true" size={18} strokeWidth={1.8} /><span className="nav-item-copy"><strong>{item.label}</strong><small>{hint}</small></span></button>
}
...
export function AppShell({ workspace, workspaces, onWorkspaceChange, activeRoute, onNavigate, onOpenDraftHub, onOpenTeamBot, onOpenCollaboration, onOpenMemory, onOpenGovernance, isTelegramMiniApp = false, telegramState, onOpenBrowser, children }: AppShellProps) {
  ...
  const explainPlannedDestination = (item: NavigationItem) => {
    setPlannedNotice(`${item.label}尚未上线：${item.description}。当前没有可用的受控后端流程，因此不会跳转。`)
  }
  ...
  return <div className="app-shell">
    <aside className="desktop-sidebar" aria-label="主导航" data-visual-system="stage07-workbench">
      ...
      <nav className="primary-nav" data-visual-system="stage07-workbench">
        {primaryItems.map((item) => <NavigationButton item={item} activeRoute={activeRoute} className="nav-item" onPlanned={explainPlannedDestination} key={item.label} />)}
        {managementItems.length > 0 && <div className="nav-divider" />}
        {managementItems.map((item) => <NavigationButton item={item} className="nav-item" onPlanned={explainPlannedDestination} key={item.label} />)}
      </nav>
