import { ArrowUpRight, Bot, CheckCircle2, ChevronRight, CircleDot, Plus, Search } from 'lucide-react'

import type { BaseSummary, Workspace, WorkspaceHome as WorkspaceHomeData } from './api'

type WorkspaceHomeProps = { home: WorkspaceHomeData; workspace: Workspace; onOpenBase: (base: BaseSummary) => void; onCreateBase?: () => void; onOpenTemplateImport?: () => void }

export function WorkspaceHome({ home, workspace, onOpenBase, onCreateBase, onOpenTemplateImport }: WorkspaceHomeProps) {
  return <main className="workspace-home" aria-label="工作区首页">
    <section className="queue-surface" aria-labelledby="today-work-heading">
      <header className="page-toolbar"><div><h1 id="today-work-heading">今天工作</h1><p>{workspace.name} · 当前需处理的持久化事项</p></div><div className="toolbar-actions">{workspace.capabilities.can_manage_schema && onCreateBase && <button type="button" onClick={onCreateBase}><Plus size={17} /> 新建 Base</button>}{workspace.capabilities.can_manage_schema && onOpenTemplateImport && <button type="button" onClick={onOpenTemplateImport}>模板与导入</button>}<button type="button"><Search size={17} /> 搜索</button><button type="button">按时间排序 <ChevronRight size={15} /></button></div></header>
    ...
    <aside className="base-rail" aria-labelledby="recent-bases-heading"><div className="rail-heading"><h2 id="recent-bases-heading">最近 Base</h2><a href="#bases">全部 <ChevronRight size={15} /></a></div><div className="base-list">{home.recent_bases.map((base, index) => <a className="base-preview" href={`#base/${base.id}`} key={base.id} aria-label={base.name} onClick={(event) => { event.preventDefault(); onOpenBase(base) }}><div className="base-preview-title"><span className={`base-glyph glyph-${index % 3}`}>▣</span><strong>{base.name}</strong></div><span className="base-kind">{base.source_type === 'blank' ? '多维表格' : base.source_type}</span><div className="preview-grid" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /></div></a>)}</div></aside>
