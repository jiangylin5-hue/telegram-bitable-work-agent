...
test('opens Stage08 collaboration from Home and sends only the safe query contract', async () => {
  const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/api/stage08/assistant/query' && init?.method === 'POST') return Promise.resolve(json({
      status: 'completed', answer: '先确认预算节点。', citations: [{ ordinal: 1, label: 'group_context' }], degradation_codes: [], draft_id: null,
      private_context: 'must not render',
    }))
