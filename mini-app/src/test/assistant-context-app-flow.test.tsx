...
test('opens the Ledgerline collaboration flow and consumes only the safe Stage08 stream result', async () => {
  const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/mini-app/workspaces/workspace-1/digital-employee-contacts?limit=50') return Promise.resolve(json({
      ...
      contacts: [{ id: 'employee-1', base_id: 'base-1', name: '运营助理', description: '仅分析授权范围。', status: 'active', available_intents: ['summarize'] }],
      next_cursor: null,
      has_more: false,
    }))
    if (path === '/api/stage08/assistant/query-stream' && init?.method === 'POST') return Promise.resolve(sse([
      ...
      { event: 'answer_delta', sequence: 2, request_id: 'request-1', text: '当前有一项需要优先复核。' },
      {
        event: 'result',
        sequence: 3,
  ...
  expect(await screen.findByTestId('collaboration-context-strip')).toBeVisible()
  fireEvent.change(screen.getByRole('textbox', { name: '协作问题' }), { target: { value: '请识别当前材料中的风险，并区分事实与分析。' } })
  ...
  expect(JSON.parse(String(invocation?.[1]?.body))).toMatchObject({
    workspace_id: 'workspace-1',
    employee_id: 'employee-1',
    intent: 'mixed',
    query: '请识别当前材料中的风险，并区分事实与分析。',
