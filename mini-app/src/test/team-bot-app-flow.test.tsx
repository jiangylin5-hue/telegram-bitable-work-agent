...
test('rereads the selected team view before one safe summary and hands off only to its Base', async () => {
  ...
  const fetchMock = vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (path === '/mini-app/team-bots/employee-1/summaries' && init?.method === 'POST') {
      summaryRequests += 1
      if (summaryRequests === 2) return Promise.resolve(json({ detail: 'idempotency_conflict' }, 409))
  ...
  fireEvent.change(screen.getByLabelText('补充说明'), { target: { value: '保留这条本地说明。' } })
  fireEvent.click(screen.getByRole('button', { name: '生成团队摘要' }))
  await waitFor(() => expect(summaryRequests).toBe(2))
  expect(screen.getByLabelText('补充说明')).toHaveValue('保留这条本地说明。')
  expect(screen.getByRole('button', { name: '选择团队视图 本周任务' })).toBeVisible()
  expect(screen.queryByText('idempotency_conflict')).not.toBeInTheDocument()
