...
- Current worktree includes the bounded F2 closure plus V1 Task 12 evidence. V1-1 through V1-12 is `partial-local`: durable ownership/grant persistence (`20260711_0022`), typed/canonical/versioned server behavior, safe routes/query execution, PostgreSQL proof, protected client panels/Canvas recovery and Browser core observations at 1440/1280/430/390 are recorded. The Browser fixture observed private Grid/Kanban/Calendar/Form flows, owner editor grant, safe `409` canonical reread, 390px focus return and console `[]`; it was removed after use. It did not switch roles, exercise every invalid/F2 state, inspect real-backend traffic or establish Telegram/staging/production evidence. Inspect `git status --short`, the V1 evidence files and implementation plan before any new change.
...
- V1 Saved View Builder design and implementation plan are user-approved; V1-1 through V1-12 is `partial-local`:
...
If continuing V1, use a disposable authorized multi-identity fixture or approved non-production backend to cover editor/viewer intersection, invalid query/F2 states and real route traffic, then re-evaluate the V1 BDD rows. Otherwise treat imports/templates, governance and Package 4 as separate decisions. No raw config/policy browser contract is authorized.
