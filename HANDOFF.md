...
- Current worktree includes the bounded F2 closure plus V1-13 evidence. V1-1 through V1-13 is `partial-local`: durable ownership/grant persistence (`20260711_0022`), typed/canonical/versioned server behavior, safe routes/query execution, PostgreSQL proof, protected panels/Canvas recovery, four-width Browser core observations and actual owner/editor/viewer surface separation are recorded. The Browser fixture also observed disabled bounded query controls, safe relation label, editor save, safe `409` canonical reread and console `[]`; it was removed after use. It did not prove real-backend Base/Table/Field intersection, every invalid/F2 state, Telegram/staging/production. Inspect `git status --short`, V1 evidence and the implementation plan before any new change.
...
- V1 Saved View Builder design and implementation plan are user-approved; V1-1 through V1-13 is `partial-local`:
...
The most recent V1-12 work records the actual Browser core matrix and its gaps without accepting V1 or Stage07.

### V1-10 authoritative Canvas lifecycle and list/direct-read ACL closure
...
- Latest local verification: focused V1 backend `24 passed`, real disposable V1 PostgreSQL `11 passed`, full backend `512 passed, 17 skipped` (historical Stage02 online-smoke only), Mini App `24` files / `114` tests and production build passed. The Browser core matrix is recorded, but no Telegram, staging or production evidence exists.
...
## 6. Immediate Next Step: Reconcile V1 Browser Gaps Or Move To A Separately Approved Scope

P3, F1 and F2 remain bounded `implemented-local` substages. V1's complete design package and TDD plan are user-approved; Tasks 1--12 have been reconciled as `partial-local`:
...
If continuing V1, use an approved non-production backend to prove actual Base/Table/Field intersection and real route traffic, then cover the remaining numeric-lookup/Record Detail/type-invalid Browser cases before re-evaluating the V1 BDD rows. Otherwise treat imports/templates, governance and Package 4 as separate decisions. No raw config/policy browser contract is authorized.
