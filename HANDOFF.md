...
- Current worktree includes the bounded F2 closure plus authorized V1 implementation. V1-1/2 has local durable ownership/grant persistence (`20260711_0022`) and strict typed command/read models; inspect `git status --short` and the V1 implementation plan before any new change.
...
- V1 Saved View Builder design and implementation plan are user-approved; V1-1/2 has begun:
  ...
  - local model/migration/UoW evidence exists for owner/scope/version and unique member grants; strict typed command/read models reject raw state and bound V1 inputs; no V1 API, authorization/configuration behavior, dependency, capability or Browser evidence exists yet.
...
The detailed TDD implementation plan is approved and V1-1/2 persistence/schema tasks are complete locally. Continue with server-owned canonical validation/projection before changing any API/browser contract. Imports/templates, governance and Package 4 remain later separate decisions. No raw config/policy browser contract is authorized.
