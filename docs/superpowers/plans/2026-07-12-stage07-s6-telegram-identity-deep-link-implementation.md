...
- The remaining plan item is external authority. The complete local App matrix now covers target-reread `401`/`403` denied, `404`/`409`/`422`/network recovery and an unmounted late resolver response. `openBase` invalidates the launch generation and the handoff no longer overwrites denied state with recovery/loading. Cross-workspace recovery and field-policy projection were reconciled against the current Stage07 API data-security contract: a field-policy change filters the later authoritative read; a workspace/resource action failure recovers.
